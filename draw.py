...
def add_sphere(polygons, cx, cy, cz, r, steps ):
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop):
            ...
            #print(str(lat) + ' ' + str(longt) + ' ' + str(index))
            if (longt == longt_start or longt == longt_stop):
                add_polygon(polygons, points[index][0], points[index][1], points[index][2], 
                    ...
                    points[(index + steps) % len(points)][0], points[(index + steps) % len(points)][1], points[(index + steps) % len(points)][2])
            
            else:
                add_polygon(polygons, points[index][0], points[index][1], points[index][2], 
                    points[(index + 1) % len(points)][0], points[(index + 1) % len(points)][1], points[(index + 1) % len(points)][2],
                    points[(index + steps + 1) % len(points)][0], points[(index + steps + 1) % len(points)][1], points[(index + steps + 1) % len(points)][2])
                add_polygon(polygons, points[index][0], points[index][1], points[index][2], 
                    points[(index + steps + 1) % len(points)][0], points[(index + steps + 1) % len(points)][1], points[(index + steps + 1) % len(points)][2],
                    points[(index + steps) % len(points)][0], points[(index + steps) % len(points)][1], points[(index + steps) % len(points)][2])                    
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop):
            ...
            add_polygon(polygons, points[index][0], points[index][1], points[index][2],
                        points[(index + steps + 1) % len(points)][0], points[(index + steps + 1) % len(points)][1], points[(index + steps + 1) % len(points)][2], 
                        points[(index + steps) % len(points)][0], points[(index + steps) % len(points)][1], points[(index + steps) % len(points)][2])
            add_polygon(polygons, points[(index + 1) % len(points)][0], points[(index + 1) % len(points)][1], points[(index + 1) % len(points)][2],
                        points[(index + steps + 1) % len(points)][0], points[(index + steps + 1) % len(points)][1], points[(index + steps + 1) % len(points)][2], 
                        points[(index) % len(points)][0], points[(index) % len(points)][1], points[(index) % len(points)][2])
